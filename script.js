...
document.addEventListener('DOMContentLoaded', () => {
   ...
   // Membuat aurora dengan partikel transparan
   const particles = [];
   const particleCount = 500;
   for (let i = 0; i < particleCount; i++) {
     const geometry = new THREE.CircleGeometry(0.05, 6);
     const material = new THREE.MeshBasicMaterial({
      color: Math.random() * 0xFFDAB9 + 0xFF69B4, // Efek warna aurora
       opacity: 0.7,
       transparent: true
     });
   ...
   function animate() {
     ...
     particles.forEach(particle => {
      particle.position.y -= 0.05; // Partikel bergerak ke bawah
      if (particle.position.y < -25) {
        particle.position.y = 25;  // Menyusun ulang posisi partikel saat mencapai batas bawah
      }
     });
...
function getGuestName() {
  ...
  //HANDLE COUNTDOWN
    const countdown = () => {
      const weddingDate = new Date("2025-05-30T00:00:00").getTime(); // Set tanggal pernikahan
